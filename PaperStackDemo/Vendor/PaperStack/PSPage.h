...
@private
  ...
  Vertex3f  *outputInnerShadowMesh_;    // Vertex array for the page (front and back combined) after being deformed by rho, theta, and A deformation parameters.
  Vertex3f  *outputOuterShadowMesh_;    // Vertex array for the page (front and back combined) after being deformed by rho, theta, and A deformation parameters.  
...
@property (nonatomic, assign) BOOL hasReverseCurl;

- (const Vertex2f *) textureArray;
- (const Vertex3f *) vertices;  // Deformed page mesh as a vertex array.
- (const Vertex3f *) innerShadowVertices;  // Deformed page mesh as a vertex array.
- (const Vertex3f *) outerShadowVertices;  // Deformed page mesh as a vertex array.
